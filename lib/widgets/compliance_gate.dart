  ...
  Future<void> _showAgreementDialog() async {
    await showDialog<void>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: const Text('AiTODO 当前版本为本地优先的任务管理工具。请先阅读并同意《隐私政策》和《用户协议》。'),
        actions: [
          TextButton(
            onPressed: () =>
                _openDocument(dialogContext, LegalDocumentType.privacyPolicy),
            child: const Text('隐私政策'),
          ),
          TextButton(
            onPressed: () =>
                _openDocument(dialogContext, LegalDocumentType.userAgreement),
